...
private func downcastingTheClosureCannotRecoverTheService(access: MeetingAsrRuntimeAccess) async {
    // expect-warning: cast from 'MeetingAsrManagerBorrow' (aka '@MainActor @Sendable () -> Optional<(manager: AsrManager, version: AsrModelVersion)>') to unrelated type 'FluidAudioTranscriptionService' always fails
    if let concrete = access.borrowLoadedManager as? FluidAudioTranscriptionService {
